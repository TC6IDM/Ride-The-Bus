...
// contextTime is the context's; primeAudio moved to audioAutoplay.ts, which
// answers the permission question rather than building the graph.
const graph = await import('../audioContext.ts');
const autoplay = await import('../audioAutoplay.ts');
const { primeAudio } = autoplay;
...
describe('before anything has opened a context', () => {
	test('nothing is downloaded until there is both a track and a context', async () => {
		...
		const stopPriming = autoplay.primeAudio();
		await advance(1);
