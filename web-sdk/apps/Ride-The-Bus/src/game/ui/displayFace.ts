...
/**
 * The two subsets the BODY FACE ALONE adds beyond latin-ext, transcribed from
 * the vietnamese and cyrillic blocks in components/app.css (the test checks).
 * Vietnamese closes the U+1EA0-1EF1 gap the latin-ext block leaves; cyrillic
 * is what lets a Russian tier name render in the game's own face.
 */
...
export const CYRILLIC_SUBSET: ReadonlyArray<Range> = [
	[0x0301, 0x0301],
	[0x0400, 0x045f],
	[0x0490, 0x0491],
	[0x04b0, 0x04b1],
...
const BODY_ONLY: ReadonlyArray<Range> = [...LATIN_EXT_SUBSET, ...VIETNAMESE_SUBSET, ...CYRILLIC_SUBSET];
 ...
 *               latin-ext, vietnamese, cyrillic).
 * - `system`  - neither covers it; hand the whole string to the OS stack, which
 *               is what ar/hi/ja/ko/zh already do and is not a regression for
 *               them.
 *
