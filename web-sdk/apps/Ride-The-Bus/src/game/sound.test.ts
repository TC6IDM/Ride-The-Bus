...
type Graph = typeof import('./audioGraph.ts');

const reload = (tag: string): Promise<Graph> => {
	const specifier = `./audioGraph.ts?${tag}`;
...
describe('the reveal loop is wired to the right cues', () => {
	...
	const loop = GAME.slice(
		GAME.indexOf('async function playRevealSequence'),
		GAME.indexOf('const multiplier = engineFinalMultiplier'),
	).replace(/\/\/.*$/gm, '');
...
describe('pressKindFor maps onto classes that exist', () => {
	...
	const GAME = read('../components/Game.svelte');
	const CSS = ['popups', 'control-bar', 'choices', 'popup-base', 'cards']
