...
import { CHIP_COLOURS, chipColour, labelEms, splitChipLabel } from './betChips.ts';
...
describe('the chrome does not slide back', () => {
  ...
  const POPUPS = read('../styles/popups.css');
  const TOKENS = read('../styles/tokens.css');
  const TABLE = read('../styles/table.css');
  const CHOICE_ICON = read('../components/ChoiceIcon.svelte');
