...
import {
  GAME_MARKUP,
  INTRO_PANELS_CSS,
  POPUP_STYLESHEET_PATHS,
  START_SCREEN_CSS,
  START_SCREEN_STYLESHEET_PATHS,
...
describe('figures are laid out left to right whatever the locale', () => {
   ...
   */
  const markup = [
    // Game.svelte is not listed here: GAME_MARKUP below is the component
    // PLUS every component its template was split into, so a readout that
    // moved into a child is still checked. See sources.testlib.ts.
    'components/popups/HowToPlayPopup.svelte',
    'components/intro/StartScreen.svelte',
...
describe('the guess squares keep their own corners in both directions', () => {
  const css = read('styles/board/choices.css');
...
describe('the intro screen fits the frame in Arabic', () => {
  // The intro's own sheet - start-screen.css became three when StartScreen
  // became three components. See sources.testlib.ts.
  const css = INTRO_PANELS_CSS;
...
describe('the loader deals from the side the row is dealt into', () => {
  const css = read('styles/intro/loader.css');
...
describe('the marks that mean a direction turn round', () => {
  const svelte = read('components/icons/MarkIcon.svelte');
...
describe('the readouts and the pay table align logically', () => {
  // Every panel sheet, not one popups.css - see sources.testlib.ts.
  const sheets = ['styles/board/control-bar.css', 'styles/popups/popup-how-to-play.css']
    .map((rel) => [rel, read(rel)] as const)
    .concat(
      [...POPUP_STYLESHEET_PATHS, ...START_SCREEN_STYLESHEET_PATHS].map(
  ...
  test('the last-win multiplier is spaced logically', () => {
    const css = read('styles/board/control-bar.css');
