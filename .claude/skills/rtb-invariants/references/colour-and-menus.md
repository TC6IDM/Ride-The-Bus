...
- **Figures hold still because the face has tabular figures — and the face
  was measured, not defaulted.** Under Poppins neither self-hosted face shipped
  ...
  were measured off their shipped files: **Poppins**, **Barlow** (kin to Big
  Shoulders' signage voice, but static — nine files for three weights — with
  no Cyrillic and no rupee glyph) and **Geist**, which is variable, ships
  `tnum` (0.648em at 800), covers Vietnamese and Cyrillic, and owns the rupee,
  peso and sheqel the readouts print. Geist was chosen for that coverage with
  its one cost recorded in `design.md`: it is the 2025 scaffold default, and
  the rest of the system is what has to carry the character. Every constant
  ...
  `displayFace.ts` — was re-measured.
